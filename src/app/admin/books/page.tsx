...
import { processBookTocAction, saveBookAction, fetchBookMetadataAction, listBooksAction, deleteBookAction, updateBookMetadataAction } from '@/app/actions';
...
export default function AdminBooksPage() {
    ...
    const [editApaCitation, setEditApaCitation] = useState('');
    const [isEditApaEdited, setIsEditApaEdited] = useState(false);
    ...
    React.useEffect(() => {
        if (!isEditApaEdited) {
            setEditApaCitation(formatApaCitation(editAuthor, editYear, editTitle, editEdition, editPublisher));
        }
    }, [editAuthor, editYear, editTitle, editEdition, editPublisher, isEditApaEdited]);
    ...
    const handleEditClick = (book: any) => {
        ...
        setEditApaCitation(book.apaCitation || '');
        const autoGenerated = formatApaCitation(book.author || '', book.year || '', book.title || '', book.edition || '', book.publisher || '');
        setIsEditApaEdited(book.apaCitation ? book.apaCitation.trim() !== autoGenerated.trim() : false);
    ...
    const handleUpdateMetadata = async () => {
        ...
        if (!editTitle.trim() || !editAuthor.trim()) {
            toast({
                title: "Mangler obligatoriske felter",
                description: "Indtast venligst både titel og forfatter.",
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            const res = await updateBookMetadataAction({
                id: editingBookId,
                title: editTitle,
                author: editAuthor,
                year: editYear,
                ...
                edition: editEdition,
                apaCitation: editApaCitation
            });

            if (res.success) {
                toast({
                    title: "Bog opdateret",
                    description: `Metadata for "${editTitle}" er blevet opdateret.`,
                });
                ...
                throw new Error(res.error || "Kunne ikke opdatere bogen.");
            }
        } catch (error: any) {
            toast({
                                    ...
                                    <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0 border-slate-100">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button 
                                                ...
                                                className="p-3 hover:bg-slate-50 border border-transparent hover:border-slate-100 rounded-xl text-slate-400 hover:text-indigo-600 transition-all flex items-center justify-center"
                ...
                {isEditModalOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div 
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            onClick={() => !isUpdatingMetadata && setIsEditModalOpen(false)}
                        ...
                        <motion.div 
                            ...
                            className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden z-10 flex flex-col max-h-[90vh]"
                        >
                            {/* Modal Header */}
                            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-[950] text-slate-900 leading-snug">Rediger bogmetadata</h2>
                                        <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider">Opdater bogoplysninger og referencer</p>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => !isUpdatingMetadata && setIsEditModalOpen(false)}
                            ...
                            {/* Modal Body */}
                            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6 custom-scrollbar-light">
                                {/* Title and Author Row */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Titel *</label>
                                        <input 
                                            type="text"
                                            value={editTitle}
                                            onChange={(e) => setEditTitle(e.target.value)}
                                            placeholder="F.eks. Sociologi: En grundbog"
                                            className="w-full h-12 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-600 outline-none transition-all"
                                            required
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Forfatter(e) *</label>
                                        <input 
                                            type="text"
                                            value={editAuthor}
                                            onChange={(e) => setEditAuthor(e.target.value)}
                                            placeholder="F.eks. Anthony Giddens"
                                            className="w-full h-12 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-600 outline-none transition-all"
                                            required
                                        />
                                    </div>
                                ...
                                {/* Year, Publisher, Edition Row */}
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Udgivelsesår</label>
                                        <input 
                                            type="text"
                                            value={editYear}
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Forlag</label>
                                        <input 
                                            type="text"
                                            value={editPublisher}
                                            onChange={(e) => setEditPublisher(e.target.value)}
                                            placeholder="F.eks. Hans Reitzels Forlag"
                                            className="w-full h-12 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-600 outline-none transition-all"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Udgave</label>
                                        <input 
                                            type="text"
                                            value={editEdition}
                                ...
                                <div className="space-y-3 pt-2">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">APA Litteraturhenvisning</label>
                                        {isEditApaEdited && (
                                            <button 
                                                onClick={() => {
                                                    setIsEditApaEdited(false);
                                                    setEditApaCitation(formatApaCitation(editAuthor, editYear, editTitle, editEdition, editPublisher));
                                                    toast({
                                                        title: "APA reference nulstillet",
                                                        description: "Henvisningen genereres nu automatisk baseret på felterne.",
                                                    });
                                                }}
                                                className="text-[10px] font-black uppercase tracking-wider text-indigo-600 hover:text-indigo-700 transition-colors"
                                            >
                                                Nulstil til automatisk APA
                                            </button>
                                        )}
                                    </div>
                                    <div className="relative group">
                                        <textarea 
                                            value={editApaCitation}
                                            onChange={(e) => {
                                                setEditApaCitation(e.target.value);
                                                setIsEditApaEdited(true);
                                            }}
                                            placeholder="APA-henvisningen genereres automatisk, men kan overskrives her..."
                                            className="w-full h-24 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-serif italic focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-600 outline-none transition-all resize-none pr-12 leading-relaxed"
                                        />
                                        <div className="absolute right-4 bottom-4 flex items-center gap-1.5 pointer-events-none select-none">
                                            {isEditApaEdited ? (
                                                <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-100 rounded-md">Manuel</span>
                                            ) : (
                                                <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-md">Auto</span>
                                            )}
                                        </div>
                                    </div>
                                    <p className="text-[10px] font-bold text-slate-400 leading-normal">
                                        Tip: Kursiv markeres med stjerner, f.eks. <span className="font-serif italic">*Titel på bogen*</span>.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="px-8 py-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
                                <Button 
                                    ...
                                    disabled={isUpdatingMetadata}
                                    className="rounded-xl h-12 px-6 font-black uppercase tracking-widest text-[10px] border-slate-200 text-slate-600 hover:bg-slate-100"
                                >
                                    Annuller
                                </Button>
                                <Button 
                                    onClick={handleUpdateMetadata}
                                    disabled={isUpdatingMetadata || !editTitle.trim() || !editAuthor.trim()}
                                    className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl h-12 px-8 font-black uppercase tracking-widest text-[10px] shadow-xl shadow-indigo-100 flex items-center gap-2"
                                >
                                    {isUpdatingMetadata ? (
                                        <>
