...
import { getTermsConfigAction } from '@/app/actions';
...
export default function TermsOfServicePage() {
    const [dynamicTerms, setDynamicTerms] = useState<{ content: string, version: string, updatedAt: string } | null>(null);

    useEffect(() => {
        async function fetchTerms() {
            const res = await getTermsConfigAction();
    ...
    return (
        <div className="bg-[#fafafa] min-h-screen selection:bg-amber-500/10 selection:text-amber-600">
            ...
            <main className="max-w-6xl mx-auto px-6 pb-40">
                {dynamicTerms && dynamicTerms.content && dynamicTerms.content !== "Standard Handelsbetingelser..." ? (
                    <Reveal delay={0.2}>
                        <div className="bg-white p-12 md:p-20 rounded-[3rem] border border-slate-100 shadow-xl space-y-12 min-h-[600px]">
                            <div className="flex items-center justify-between border-b border-slate-50 pb-8">
                                <div className="flex items-center gap-4">
                                    ...
                                    <h3 className="text-xl font-black text-slate-900 serif">Gældende Betingelser</h3>
                                </div>
                                <div className="px-4 py-2 bg-slate-50 rounded-full text-[10px] font-black uppercase text-slate-400">
                                    Versio {dynamicTerms.version}
                                </div>
                            </div>
                ...
                <div className="mt-20 text-center">
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">
                        Sidst opdateret: {dynamicTerms?.updatedAt ? new Date(dynamicTerms.updatedAt).toLocaleDateString('da-DK', { day: '2-digit', month: 'long', year: 'numeric' }) : '26. marts 2026'}
                    </p>
                </div>
            </main>
