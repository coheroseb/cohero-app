...
import { syncCalendarAvailability, extractTasksFromTextAction, analyzeTaskScheduleAction, sendGroupInvitationEmailAction } from '@/app/actions';
...
interface MemberAvailability {
    id: string;
    username: string;
    slots: Record<string, AvailabilityMode>; // key pattern: 'day-time' e.g. 'mandag-morning'
    updatedAt: any;
    icalUrl?: string;
}
...
export default function GroupDetailPage() {
  ...
  const handleAnalyzeSchedule = async () => {
    ...
    try {
        ...
        const result = await analyzeTaskScheduleAction({ tasks: cleanTasks });
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    if (!email || !firestore || isSaving || !user || !group) return;
    ...
    try {
      ...
      if (snap.empty) {
          toast({ 
              ...
              description: "Kunne ikke finde en bruger med denne e-mail." 
          });
          setIsSaving(false);
          return;
      }
      ...
      const targetUserId = foundUserDoc.id;
      const targetUserName = foundUserDoc.data().username || 'Kollega';

      if (group?.memberIds.includes(targetUserId)) {
          toast({ 
              variant: 'destructive', 
              title: "Allerede medlem", 
              description: "Denne kollega er allerede tilføjet til gruppen." 
          });
          setIsSaving(false);
          return;
      }
      ...
      const memberDocRef = doc(firestore, 'groups', groupId, 'members', targetUserId);
      ...
      const parentGroupRef = doc(firestore, 'groups', groupId);
      batch.update(parentGroupRef, {
      ...
      sendGroupInvitationEmailAction({
          ...
          groupUrl: `${window.location.origin}/grupper/${groupId}`
      });
  ...
  const handleToggleSlot = (dayId: string, slotId: string) => {
      ...
      const currentValue = currentSlots[key] || null;

      let nextValue: AvailabilityMode = null;
      if (currentValue === null) nextValue = 'online';
      else if (currentValue === 'online') nextValue = 'physical';
      else if (currentValue === 'physical') nextValue = 'unavailable';
      else nextValue = null;

      const newSlots = { ...currentSlots, [key]: nextValue };
  ...
  const handleSyncIcal = async (e?: React.FormEvent) => {
      ...
      setIsSyncing(true);
      try {
          const syncedSlots = await syncCalendarAvailability(icalUrlInput.trim());
          const userAvail = availabilities?.find(a => a.id === user.uid);
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] flex flex-col lg:flex-row text-slate-900 selection:bg-indigo-100 overflow-x-hidden">
      ...
      <aside className="w-full lg:w-80 bg-white border-r border-amber-100 flex flex-col h-screen z-30 shadow-sm shrink-0">
        <div className="p-8 flex items-center gap-4 border-b border-amber-50">
            <button onClick={() => router.push('/grupper')} className="p-2 bg-amber-50 text-amber-900 rounded-xl hover:bg-amber-100 transition-all shrink-0">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <div className="min-w-0">
                <h1 className="text-lg font-bold text-amber-950 serif tracking-tight truncate">{group.name}</h1>
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-900/40">Studiegruppe</p>
            </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            <nav className="p-4 space-y-1">
                <button onClick={() => setActiveTab('tasks')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'tasks' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><ClipboardList className="w-4 h-4" /> Opgaver</div>
                ...
                <button onClick={() => setActiveTab('timeline')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'timeline' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><GanttChart className="w-4 h-4" /> Tidslinje</div>
                </button>
                <button onClick={() => setActiveTab('evidence')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'evidence' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><FileSearch className="w-4 h-4" /> Empiri</div>
                ...
                <button onClick={() => setActiveTab('contacts')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'contacts' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><Contact className="w-4 h-4" /> Kontaktbog</div>
                ...
                <button onClick={() => setActiveTab('availability')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'availability' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><CalendarCheck className="w-4 h-4" /> Tilgængelighed</div>
                </button>
                <button onClick={() => setActiveTab('members')} className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeTab === 'members' ? 'bg-amber-950 text-white shadow-xl' : 'text-slate-500 hover:bg-amber-50'}`}>
                    <div className="flex items-center gap-3"><Users className="w-4 h-4" /> Medlemmer</div>
            ...
            <div className="mt-auto p-6 border-t border-amber-50 bg-amber-50/20">
                {deadlineInfo && (
                    <div className="mb-6 p-4 bg-white rounded-2xl border border-amber-100 shadow-sm animate-ink">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Fælles Deadline</span>
                        </div>
                        <p className="text-sm font-black text-amber-950 serif">{deadlineInfo.date.toLocaleDateString('da-DK', { day: 'numeric', month: 'long' })}</p>
                        <p className={`text-[10px] font-bold mt-1 ${deadlineInfo.daysLeft < 7 ? 'text-rose-600 animate-pulse' : 'text-emerald-600'}`}>
                            {deadlineInfo.daysLeft > 0 ? `${deadlineInfo.daysLeft} dage tilbage` : deadlineInfo.daysLeft === 0 ? 'Deadline i dag!' : 'Deadline overskredet'}
                        </p>
                    </div>
                )}
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-[10px] font-black uppercase text-amber-900 tracking-widest">Om gruppen</h4>
                    </div>
                    {isAdmin && (
                        <button 
                            onClick={() => {
                ...
                <p className="text-xs text-slate-500 italic leading-relaxed">{group.description || 'Ingen beskrivelse tilføjet.'}</p>
            </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-white border-b border-amber-100 px-8 flex items-center justify-between shrink-0 shadow-sm">
            <h2 className="text-lg font-bold text-amber-950 serif uppercase tracking-widest">
                {activeTab === 'tasks' ? 'Opgaver & Planlægning' : 
                 activeTab === 'timeline' ? 'Gantt-Diagram' : 
                 activeTab === 'members' ? 'Dine Kolleger' : 
                 activeTab === 'evidence' ? 'Empiri-Arkiv' : 
                 activeTab === 'contacts' ? 'Kontaktbog' :
                 'Ugeplan & Tilgængelighed'}
            </h2>
            <div className="flex items-center gap-3">
                {activeTab === 'timeline' && (
                ...
                {activeTab === 'tasks' && (
                    <>
                        <div className="flex items-center bg-slate-50 p-1 rounded-xl border border-amber-100 mr-2">
                            ...
                            <button onClick={() => setTaskViewMode('board')} className={`p-2 rounded-lg transition-all ${taskViewMode === 'board' ? 'bg-white text-amber-950 shadow-sm' : 'text-slate-400 hover:text-amber-950'}`}><LayoutGrid className="w-4 h-4" /></button>
                        </div>
                        <label className="cursor-pointer">
                            <div className="px-4 h-10 bg-amber-50 text-amber-900 border border-amber-200 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-amber-100 transition-all">
                                ...
                                {isImportingTasks ? 'Analyserer...' : 'Importér PDF'}
                            </div>
                        ...
                        <Button onClick={() => setIsAddingTask(true)} size="sm" className="rounded-xl h-10 shadow-lg">
                            <Plus className="w-4 h-4 mr-2"/> Ny opgave
                        </Button>
                    </>
                )}
                {activeTab === 'members' && (
                    <Button onClick={() => setIsAddingMember(true)} size="sm" className="rounded-xl h-10 shadow-lg">
                        <UserPlus className="w-4 h-4 mr-2"/> Inviter kollega
                    </Button>
                )}
            </div>
        </header>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'tasks' && (
                    <motion.div key="tasks" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto space-y-12">
                                                ...
                                                <div className="flex items-center gap-6 flex-1 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        <div className="flex items-center gap-3 mb-1">
                                                            ...
                                                            {task.dueDate && <span className="text-[8px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {new Date(task.dueDate).toLocaleDateString('da-DK')}</span>}
                                                        </div>
                                                ...
                                                <div key={task.id} className="bg-white p-6 rounded-[2rem] border border-amber-100 shadow-sm hover:border-amber-950 hover:shadow-xl transition-all group">
                                                    ...
                                                    <div className="flex items-center justify-between pt-4 border-t border-amber-50">
                                                        <div className="flex items-center gap-2">
                                                            <div className="w-6 h-6 rounded-lg bg-amber-50 flex items-center justify-center text-amber-900 text-[10px] font-black uppercase">{task.assignedToName?.charAt(0) || '?'}{task.assignedToName?.charAt(0) || ''}</div>
                                                            {task.dueDate && <span className="text-[8px] font-bold text-slate-400">{new Date(task.dueDate).toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}</span>}
                                                        </div>
                        ...
                        {(!tasks || tasks.length === 0) && !isAddingTask && (<div className="py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-amber-100 shadow-inner"><ClipboardList className="w-16 h-16 text-amber-100 mx-auto mb-6" /><h3 className="text-xl font-bold text-amber-900/40 serif">Ingen opgaver endnu</h3><p className="text-sm text-slate-400 mb-8">Start jeres samarbejde ved at tilføje den første opgave eller importér fra et dokument.</p><div className="flex gap-4 justify-center"><Button onClick={() => setIsAddingTask(true)}>Opret opgave</Button><label className="cursor-pointer"><div className="px-6 h-11 bg-amber-50 text-amber-900 border border-amber-200 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-amber-100 transition-all">Importér fra dokument</div><input type="file" className="sr-only" accept=".pdf" onChange={handleImportTasks} disabled={isImportingTasks} /></label></div></div>)}
                    </motion.div>
                )}

                {activeTab === 'timeline' && (
                    <motion.div key="timeline" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-6xl mx-auto">
                        {!ganttData ? (
                            ...
                            <div className="bg-white rounded-[3rem] border border-amber-100 shadow-xl overflow-hidden">
                                <div className="p-8 border-b border-amber-50 bg-slate-50/50 flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                                        <span>Start: {ganttData.start.toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}</span>
                                        <div className="w-1 h-1 bg-amber-200 rounded-full"></div>
                                        <span>Slut: {ganttData.end.toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}</span>
                                    </div>
                                </div>
                                            ...
                                            <div className="flex">
                                                {ganttData.days.map((day, i) => (
                                                    <div key={i} className={`w-12 shrink-0 p-4 text-center border-r border-amber-50 flex flex-col items-center justify-center ${day.getDay() === 0 || day.getDay() === 6 ? 'bg-amber-50/20' : ''}`}>
                                                        <span className="text-[8px] font-black text-slate-300 uppercase tracking-tighter mb-1">{day.toLocaleDateString('da-DK', { weekday: 'short' }).charAt(0)}</span>
                                ...
                                <div className="p-6 bg-slate-50 border-t border-amber-100 flex items-center gap-8 justify-center">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Færdig</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-3 h-3 rounded-full bg-indigo-600" />
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">I gang</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-3 h-3 rounded-full bg-slate-200" />
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Planlagt</span>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <motion.div key="evidence" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-5xl mx-auto space-y-8">
                        ...
                        {isAddingEvidence && (
                            <form onSubmit={handleAddEvidence} className="bg-white p-10 rounded-[3rem] border border-amber-950 shadow-2xl animate-ink space-y-6">
                                <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold text-amber-950 serif">Tilføj Empiri</h3><button type="button" onClick={() => setIsAddingEvidence(false)} className="p-2 text-slate-300 hover:text-rose-600"><X className="w-6 h-6"/></button></div>
                                <div className="space-y-4">
                                    <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Titel på data/kilde</label><Input autoFocus value={newEvidence.title} onChange={e => setNewEvidence({...newEvidence, title: e.target.value})} placeholder="F.eks. Interview med socialrådgiver..." required className="h-12 rounded-xl" /></div>
                                    <div className="grid sm:grid-cols-2 gap-6">
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Type empiri</label><select value={newEvidence.type} onChange={e => setNewEvidence({...newEvidence, type: e.target.value as any})} className="w-full h-12 px-4 bg-white border border-input rounded-xl text-sm appearance-none focus:outline-none focus:ring-2 focus:ring-amber-950"><option value="interview">Interview</option><option value="survey">Spørgeskema</option><option value="observation">Observation</option><option value="document">Dokumentanalyse</option><option value="other">Andet</option></select></div>
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Link til rådata (valgfri)</label><Input type="url" value={newEvidence.url} onChange={e => setNewEvidence({...newEvidence, url: e.target.value})} placeholder="https://..." className="h-12 rounded-xl" /></div>
                                    </div>
                                    <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Beskrivelse/Noter</label><Textarea value={newEvidence.description} onChange={e => setNewNodeText(e.target.value)} placeholder="Hvad er de vigtigste fund?..." className="rounded-2xl min-h-[120px]" /></div>
                                </div>
                                <div className="flex gap-4 pt-4"><Button type="submit" className="flex-1 h-14 rounded-2xl" disabled={isSaving}>{isSaving ? <Loader2 className="animate-spin w-5 h-5" /> : 'Tilføj til arkivet'}</Button><Button type="button" variant="ghost" onClick={() => setIsAddingEvidence(false)} className="h-14 rounded-2xl">Afbryd</Button></div>
                            </form>
                        )}
                                ...
                                return (
                                    <div key={item.id} className="bg-white p-8 rounded-[2.5rem] border border-amber-100 shadow-sm flex flex-col group hover:shadow-xl transition-all relative overflow-hidden h-full">
                                        ...
                                        <div className="relative z-10 flex flex-col h-full">
                                            ...
                                            <div className="pt-6 border-t border-amber-50 flex items-center justify-between mt-auto"><div><p className="text-[8px] font-black uppercase tracking-widest text-slate-300">Tilføjet af</p><p className="text-[10px] font-bold text-amber-900">{item.addedByName}</p></div>{item.url && (<a href={item.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-800 transition-colors">Åbn rådata <ExternalLink className="w-3 h-3" /></a>)}</div>
                                        </div>
                                    </div>
                                )
                            })}
                            {(!evidence || evidence.length === 0) && !isAddingEvidence && (<div className="col-span-full py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-amber-100 shadow-inner"><FileSearch className="w-16 h-16 text-amber-100 mx-auto mb-6" /><h3 className="text-xl font-bold text-amber-900/40 serif">Arkivet er tomt</h3><p className="text-sm text-slate-400 mb-8 max-w-xs mx-auto">Saml alle jeres kilder ét sted.</p><Button onClick={() => setIsAddingEvidence(true)}>Tilføj jeres første empiri</Button></div>)}
                        </div>
                    </motion.div>
                )}
                    ...
                    <motion.div key="contacts" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-5xl mx-auto space-y-8">
                        {isAddingContact && (
                            <form onSubmit={handleAddContact} className="bg-white p-10 rounded-[3rem] border-2 border-amber-950 shadow-2xl animate-ink space-y-6">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-xl font-bold text-amber-950 serif">Ny Kontakt</h3>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div className="space-y-4">
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Navn</label><Input value={newContact.name} onChange={e => setNewContact({...newContact, name: e.target.value})} placeholder="Fulde navn..." required className="h-12 rounded-xl" /></div>
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Titel/Rolle</label><Input value={newContact.title} onChange={e => setNewContact({...newContact, title: e.target.value})} placeholder="F.eks. Informant, Leder..." className="h-12 rounded-xl" /></div>
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Organisation</label><Input value={newContact.organization} onChange={e => setNewContact({...newContact, organization: e.target.value})} placeholder="F.eks. Københavns Kommune..." className="h-12 rounded-xl" /></div>
                                    </div>
                                    <div className="space-y-4">
                                        ...
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Telefon</label><Input type="tel" value={newContact.phone} onChange={e => setNewContact({...newContact, phone: e.target.value})} placeholder="+45 00 00 00 00" className="h-12 rounded-xl" /></div>
                                        <div><label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Noter</label><Textarea value={newContact.notes} onChange={e => setNewContact({...newContact, notes: e.target.value})} placeholder="Vigtige detaljer..." className="rounded-xl h-12" /></div>
                                    </div>
                                </div>
                                <div className="flex gap-4 pt-4">
                                    <Button type="submit" className="flex-1 h-14 rounded-2xl" disabled={isSaving}>Gem Kontakt</Button>
                                    <Button type="button" variant="ghost" onClick={() => setIsAddingContact(false)} className="h-14 rounded-2xl">Afbryd</Button>
                                </div>
                            </form>
                        )}
                        ...
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
                                ...
                                <div key={contact.id} className="bg-white p-8 rounded-[2.5rem] border border-amber-100 shadow-sm flex flex-col group hover:shadow-xl transition-all relative overflow-hidden h-full">
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <p className="text-xs font-black uppercase text-amber-700/60 mb-4">{contact.title || 'Kontakt'}</p>
                            ...
                            {(!contacts || contacts.length === 0) && !isAddingContact && (
                                <div className="col-span-full py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-amber-100 shadow-inner">
                                    <Contact className="w-16 h-16 text-amber-100 mx-auto mb-6" />
                                    <h3 className="text-xl font-bold text-amber-900/40 serif">Kontaktbogen er tom</h3>
                                    <p className="text-sm text-slate-400 mb-8">Saml alle relevante personer og informanter ét sted.</p>
                                    <Button onClick={() => setIsAddingContact(true)}>Tilføj jeres første kontakt</Button>
                                </div>
                            )}
                        </div>
                    </motion.div>
                ...
                {activeTab === 'availability' && (
                    <motion.div key="availability" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto space-y-12">
                        {showIcalInput && (
                            <form onSubmit={handleSyncIcal} className="bg-white p-8 rounded-[2.5rem] border border-amber-950 shadow-2xl animate-ink space-y-6">
                                <div className="flex justify-between items-center mb-2"><h3 className="text-xl font-bold text-amber-950 serif">Synkronisér Kalender</h3><button type="button" onClick={() => setShowIcalInput(false)} className="p-2 text-slate-300 hover:text-rose-600"><X className="w-6 h-6"/></button></div>
                                <p className="text-sm text-slate-500 italic">Indsæt dit iCal-link for at udfylde din ugeplan automatisk.</p>
                                <div className="flex flex-col sm:flex-row gap-4"><div className="relative flex-1"><LinkIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><Input autoFocus type="url" value={icalUrlInput} onChange={e => setIcalUrlInput(e.target.value)} placeholder="webcal://..." className="h-14 pl-11 rounded-xl" /></div><Button type="submit" disabled={isSyncing || !icalUrlInput.trim()} className="h-14 px-10 rounded-xl">{isSyncing ? <Loader2 className="animate-spin w-5 h-5 mr-2" /> : <Sparkles className="w-5 h-5 mr-2" />}{isSyncing ? 'Synkroniserer...' : 'Synkronisér nu'}</Button></div>
                                <div className="bg-blue-50 p-4 rounded-xl flex gap-3 items-start border border-blue-100"><Info className="w-5 h-5 text-blue-600 mt-0.5" /><p className="text-[10px] text-blue-800 leading-relaxed font-medium">Systemet analyserer din kalender for den nuværende måned og finder dine faste mønstre.</p></div>
                            </form>
                        )}
                        <section className="bg-amber-950 p-8 sm:p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group"><div className="absolute top-0 right-0 p-8 opacity-10 group-hover:rotate-12 transition-transform"><Sparkles className="w-32 h-32" /></div><div className="relative z-10 grid md:grid-cols-2 gap-10 items-center"><div className="space-y-4"><div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-400 text-amber-950 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-lg"><Zap className="w-3.5 h-3.5" /> Planlægning</div><h3 className="text-2xl font-bold serif leading-tight">Klik direkte i ugeplanen <br/> for at markere din tid.</h3><p className="text-amber-100/60 text-sm italic">Find dit navn i tabellen nedenfor og klik på tidsblokkene.</p></div><div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm grid grid-cols-2 gap-4"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-amber-400 flex items-center justify-center text-amber-950 shadow-sm"><MapPin className="w-4 h-4" /></div><span className="text-[10px] font-bold uppercase text-amber-50">Fysisk</span></div><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white shadow-sm"><Laptop className="w-4 h-4" /></div><span className="text-[10px] font-bold uppercase text-amber-50">Online</span></div><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-rose-600 flex items-center justify-center text-white shadow-sm"><UserX className="w-4 h-4" /></div><span className="text-[10px] font-bold uppercase text-amber-50">Optaget</span></div></div></div></section>
                        <section className="bg-white rounded-[3rem] border border-amber-100 shadow-xl overflow-hidden animate-ink"><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-amber-50"><tr><th className="px-8 py-8 sticky left-0 bg-white/95 backdrop-blur-md z-20 border-r border-amber-50">Kollega</th>{WEEKDAYS.map(day => (<th key={day.id} className="px-4 py-8 border-l border-amber-50"><div className="text-center"><p className="mb-2 text-amber-950 text-xs">{day.label}</p><div className="flex justify-center gap-1.5 text-[8px] opacity-40"><span>F</span><span>E</span><span>A</span></div></div></th>))}</tr></thead><tbody className="divide-y divide-amber-50">{members?.map(member => { const isMe = member.id === user?.uid; const avail = availabilities?.find(a => a.id === member.id); return (<tr key={member.id} className={`group ${isMe ? 'bg-amber-50/20' : 'hover:bg-amber-50/10'}`}><td className={`px-8 py-6 sticky left-0 z-10 border-r border-amber-50 transition-colors ${isMe ? 'bg-amber-50/80 font-black' : 'bg-white group-hover:bg-amber-50/10'}`}><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs shadow-inner ${isMe ? 'bg-amber-950 text-amber-400' : 'bg-slate-100 text-slate-400'}`}>{member.email.charAt(0).toUpperCase()}</div><div className="min-w-0"><span className={`text-sm block truncate max-w-[120px] ${isMe ? 'text-amber-950' : 'text-slate-600'}`}>{member.nickname || member.email.split('@')[0]}</span>{isMe && <span className="text-[8px] font-black uppercase tracking-tighter text-amber-700/50">Dig</span>}</div></div></td>{WEEKDAYS.map(day => (<td key={day.id} className="px-4 py-6 border-l border-amber-50"><div className="flex justify-center gap-2">{TIME_SLOTS.map(slot => { const mode = avail?.slots?.[`${day.id}-${slot.id}`] || null; const SlotIcon = mode === 'physical' ? MapPin : mode === 'online' ? Laptop : mode === 'unavailable' ? UserX : null; if (isMe) { return (<button key={slot.id} onClick={() => handleToggleSlot(day.id, slot.id)} className={`w-10 h-14 rounded-xl transition-all shadow-sm border flex items-center justify-center relative group/slot active:scale-90 ${mode === 'physical' ? 'bg-amber-400 border-amber-400 text-amber-950' : mode === 'online' ? 'bg-indigo-600 border-indigo-600 text-white' : mode === 'unavailable' ? 'bg-rose-600 border-rose-600 text-white' : 'bg-white border-amber-100 text-amber-100 hover:border-amber-300'}`}>{SlotIcon ? <SlotIcon className="w-4 h-4" /> : <div className="w-1 h-1 rounded-full bg-current opacity-30" />}<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/slot:opacity-100 bg-black/5 rounded-xl transition-opacity pointer-events-none"></div></button>) } return (<div key={slot.id} className={`w-10 h-14 rounded-xl transition-all shadow-sm border flex items-center justify-center ${mode === 'physical' ? 'bg-amber-400 border-amber-400 text-amber-950' : mode === 'online' ? 'bg-indigo-600 border-indigo-600 text-white' : mode === 'unavailable' ? 'bg-rose-600 border-rose-600 text-white' : 'bg-slate-50 border-slate-100 opacity-20'}`}>{SlotIcon && <SlotIcon className="w-4 h-4" />}</div>) })}</div></td>))}</tr>) })}</tbody></table></div></section>
                    </motion.div>
                )}
                ...
                {activeTab === 'members' && (
                    <motion.div key="members" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-4xl mx-auto space-y-8">
                        {isAddingMember && (<form onSubmit={handleAddMember} className="bg-indigo-950 p-10 rounded-[3rem] shadow-2xl animate-ink flex flex-col gap-6 relative overflow-hidden"><div className="absolute top-0 right-0 p-8 opacity-10"><UserPlus className="w-32 h-32" /></div><div className="relative z-10"><h3 className="text-xl font-bold text-white serif mb-2">Inviter en kollega</h3><div className="flex flex-col sm:flex-row gap-4"><Input autoFocus type="email" value={newMemberEmail} onChange={e => setNewMemberEmail(e.target.value)} placeholder="kollega@studie.dk" className="h-14 rounded-xl bg-white/10 border-white/20 text-white" /><Button type="submit" disabled={isSaving}>{isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Tilføj'}</Button></div><button type="button" onClick={() => setIsAddingMember(false)} className="text-indigo-300 text-xs font-bold uppercase mt-4 hover:text-white transition-colors">Annuller</button></div></form>)}
                        <div className="grid sm:grid-cols-2 gap-6">{members?.map(member => (<div key={member.id} className="bg-white p-8 rounded-[2.5rem] border border-amber-100 shadow-sm flex items-center gap-6 group hover:border-indigo-950 transition-all relative"><div className="w-16 h-16 bg-indigo-50 text-indigo-700 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner group-hover:scale-110 transition-transform">{member.email.charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="font-bold text-amber-950 truncate text-lg leading-tight">{member.nickname || member.email}</p><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1.5">{member.role === 'admin' ? 'Gruppeadministrator' : 'Kollega'}</p></div>
                        {member.id === user?.uid && (
                        ...
                        {isAdmin && member.id !== user?.uid && (<button onClick={() => handleRemoveMember(member)} disabled={isRemovingId === member.id} className="absolute top-4 right-4 p-2 text-slate-200 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">{isRemovingId === member.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <UserMinus className="w-4 h-4" />}</button>)}</div>))}</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
