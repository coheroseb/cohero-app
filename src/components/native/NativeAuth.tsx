...
export const NativeAuth = () => {
  const { handleLogin, handleSignup, handleGoogleLogin } = useApp();
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col px-8 pt-20 pb-12 font-sans selection:bg-rose-100">
      ...
      >
        ...
        <div className="space-y-2">
           ...
           <p className="text-slate-400 font-medium text-lg leading-relaxed">
             {mode === 'signin' ? 'Log ind på din Cohéro Student konto.' : 'Få din egen intelligente kollega i lommen.'}
           </p>
        </div>
      </motion.div>
      ...
      <div className="flex bg-slate-50 p-1.5 rounded-[2.5rem] mb-10 relative">
        <button 
          onClick={() => { setMode('signin'); triggerHapticFeedback(ImpactStyle.Light); }}
        ...
        <button 
          onClick={() => { setMode('signup'); triggerHapticFeedback(ImpactStyle.Light); }}
      ...
      <form onSubmit={handleSubmit} className="flex-1 space-y-5">
        ...
        >
          {isLoading ? (
            ...
            <>
              <span className="text-[13px] font-black uppercase tracking-widest">{mode === 'signin' ? 'Log Ind' : 'Opret Konto'}</span>
