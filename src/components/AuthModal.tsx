...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialState = 'signup', priceId: initialPriceId }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FDFCF8] w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden grid md:grid-cols-2">
        
        <div className="bg-amber-950 p-12 text-white hidden md:flex flex-col justify-between relative overflow-hidden">
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-12">
               {isGroupsSource ? <Users className="w-6 h-6 text-amber-400" /> : <Sparkles className="w-6 h-6 text-amber-400" />}
          ...
          <div className="relative z-10 text-xs text-amber-100/40 uppercase tracking-widest font-bold">
            {isGroupsSource ? 'Gør jeres studiegruppe professionel' : 'Sammen skaber vi bedre socialrådgivere'}
          </div>
        ...
        <div className="p-8 md:p-12 relative">
          <button 
            onClick={onClose}
            className="absolute top-6 right-6 p-2 text-amber-900/40 hover:text-amber-900 transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <div className="max-w-xs mx-auto md:mx-0">
            <div className="mb-10">
              <h3 className="text-3xl font-bold text-amber-950 serif mb-2">
             ...
             <button
                ...
                disabled={loading}
                className="w-full mb-4 py-3 bg-white border border-slate-300 text-slate-700 rounded-xl font-bold hover:bg-slate-50 transition-all disabled:opacity-50 flex items-center justify-center group"
            >
                {loading ? (
            ...
            <div className="flex items-center my-6">
                ...
                <span className="mx-4 text-xs font-bold text-slate-400">ELLER</span>
                <hr className="flex-grow border-amber-100/80"/>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-white border border-amber-100 rounded-xl focus:ring-2 focus:ring-amber-900 focus:outline-none transition-all text-sm h-12"
                  required
                />
              </div>
              <p className="text-[10px] text-slate-400 pl-2">Vi deler aldrig din e-mail.</p>
              ...
              {error && <p className="text-xs text-red-500 text-center pt-2"></p>}

              <button 
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-amber-950 text-white rounded-xl font-bold hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center group !mt-6"
              >
                {loading ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                ) : (
                  <>
                    <span>{pendingPriceId ? 'Opret & Fortsæt' : (isLogin ? 'Log ind' : 'Opret profil')}</span>
            ...
            <div className="mt-8 text-center md:text-left">
              <p className="text-xs text-slate-400 mb-2">
                 {!isLogin && <>Ved at oprette dig accepterer du vores <Link href="/terms-of-service" className="underline hover:text-amber-800">handelsbetingelser</Link> og vores <Link href="/etik" className="underline hover:text-amber-800">retningslinjer for etik og anvendelse</Link>.</>}
              </p>
              <button 
                onClick={() => {
                  setIsLogin(!isLogin);
