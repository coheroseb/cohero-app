...
// Read .env.local manually to get the service account
const envContent = await readFile(join(__dirname, '../.env.local'), 'utf8');
const match = envContent.match(/GOOGLE_SERVICE_ACCOUNT_JSON='(.*?)'/s);
if (!match) {
  throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON ikke fundet i .env.local");
}

let serviceAccountStr = match[1];
...
async function updateSearchFields() {
  // Use a collection group or just the institutions collection
  ...
  for (const doc of snapshot.docs) {
    ...
    // Kombineret søgefelt for både navn, type og emne (problemstillinger)
